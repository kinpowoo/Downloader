...
import static com.download.sample.downloader.DownloadTask.STATUS_PAUSED;
import static com.download.sample.downloader.Downloader.DOWNLOAD_MESSAGE;
import static com.download.sample.downloader.Downloader.ERROR_LOAD;
import static com.download.sample.downloader.Downloader.ERROR_USER_CANCEL;
import static com.download.sample.downloader.Downloader.ERROR_USER_PAUSE;
import static com.download.sample.downloader.Downloader.SUCCESSFUL;
    ...
    public boolean submit(DownloadTask downloadTask) {
        ...
        synchronized (mLock) {
            ...
            if(downloadTask.getFile() == null){
                ...
                downloadTask.setFile(new File(downloadTask.getContext().getExternalCacheDir(),fileName),
                        downloadTask.getContext().getPackageName()+".DownloadFileProvider");
                downloadTask.setEnableIndicator(true);
            }
            downloadTask.setCalculateMD5(false);
    ...
    private class DownloadStartTask implements Runnable {
        ...
        public void run() {
            try {
                ...
                mDownloadTask.setStatus(DownloadTask.STATUS_PENDING);
            ...
            threadPoolExecutor.execute(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        throwable.printStackTrace();
                        mDownloadTask.error(throwable.getLocalizedMessage());
                        releaseTask(mDownloadTask);
                    }
                }
            });
            ...
            try {
                ...
                if (mResult > SUCCESSFUL) {
                    if (null != mDownloadNotifier) {
                        ...
                        if(mDownloadTask.getErrorReason()!=null){
                            mDownloadNotifier.onDownloadErrorFinished(mDownloadTask.getErrorReason());
                        }else{
                            mDownloadNotifier.onDownloadErrorFinished();
                        }
                    }
                    return;
                }
                ...
                if (null != mDownloadNotifier) {
                    mDownloadNotifier.onDownloadErrorFinished(throwable.getLocalizedMessage());
                }
