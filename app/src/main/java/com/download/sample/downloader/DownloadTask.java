...
public class DownloadTask extends Extra implements Serializable, Cloneable {
    ...
    @IntDef({STATUS_NEW, STATUS_PENDING, STATUS_DOWNLOADING, STATUS_PAUSING, STATUS_PAUSED, STATUS_SUCCESSFUL, STATUS_CANCELED, STATUS_ERROR})
    ...
    void updateTime(long beginTime) {
        ...
        if (this.beginTime != beginTime) {
            detailTime += Math.abs(beginTime - this.pauseTime);
        }
    }

    public long getUsedTime() {
        if (status == STATUS_DOWNLOADING) {
            return beginTime > 0L ? SystemClock.elapsedRealtime() - beginTime - detailTime : 0L;
        } else if (status == STATUS_CANCELED) {
            return endTime - beginTime - detailTime;
        } else if (status == STATUS_PENDING) {
            return pauseTime > 0L ? pauseTime - beginTime - detailTime : 0L;
        } else if (status == STATUS_PAUSED || status == STATUS_PAUSING) {
            return pauseTime - beginTime - detailTime;
