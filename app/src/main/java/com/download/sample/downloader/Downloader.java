...
import static com.download.sample.downloader.FileComparator.COMPARE_RESULT_REDOWNLOAD_COVER;
import static com.download.sample.downloader.FileComparator.COMPARE_RESULT_SUCCESSFUL;
            ...
            for (int i = 0; i <= downloadTask.retry; i++) {
                ...
                if (ioException == null) {
                    ...
                    if (i == downloadTask.retry) {
                        downloadTask.error("io异常:"+ioException.getLocalizedMessage());
                ...
                switch (responseCode) {
                    ...
                    case HTTP_PARTIAL:
                        ...
                        if (this.mTotals > 0L && contentLength + downloadTask.getFile().length() != this.mTotals) {  // 服务端响应文件长度不正确，或者本地文件长度被修改。
                            downloadTask.error("服务端响应文件长度不正确，或者本地文件长度被修改");
                    ...
                    case HTTP_BAD_GATEWAY:
                        downloadTask.error("网关异常");
                        return ERROR_SERVICE;
                    default:
                        downloadTask.error("响应状态不正确");
                        return ERROR_RESPONSE_STATUS;
                }
            }
            downloadTask.error("重定向次数超出限制");
    ...
    protected void onProgressUpdate(Integer... values) {
        ...
        try {
            ...
            if (values != null && values.length > 0 && values[0] == 1) {
                if (null != downloadNotifier) {
                    if (mTotals > 0) {
                        int mProgress = (int) ((mLastLoaded + mLoaded) / (float) mTotals * 100);
        ...
        try {
            ...
            while (!downloadTask.isPausing() && !downloadTask.isCanceled() && !downloadTask.isPaused()) {
                ...
                try {
                    ...
                    downloadTask.error("io异常:"+e.getLocalizedMessage());
                    throw e;
                }
                if (n == -1) {
                    break;
